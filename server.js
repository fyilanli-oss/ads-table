...
app.get("/api/config", (req, res) => {
  res.json({
    metaConfigured: Boolean(META_APP_ID && META_APP_SECRET && META_REDIRECT_URI),
...
app.get("/api/meta/insights", async (req, res) => {
  try {
    ...
    res.json({
      platform: "Meta",
      range,
      campaigns: (data.data || []).map(parseInsight)
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.listen(PORT, () => {
  console.log(`Ads Table Meta running on http://localhost:${PORT}`);
});
